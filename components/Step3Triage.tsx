...
import { DepartmentRecommendation } from '../types';
...
export const Step3Triage: React.FC<Props> = ({ onRecommendation, onBack }) => {
  ...
  const handleAnalyze = async () => {
    ...
    setLoading(true);
    try {
      const rec = await analyzeSymptoms(symptoms);
      setResult(rec);
    } catch (e) {
      console.error(e);
    } finally {
        ...
        <div className="flex flex-col gap-4">
          <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-200">
            ...
            <textarea
              ...
              placeholder="Ví dụ: Tôi bị đau bụng dữ dội vùng dưới bên phải, kèm sốt nhẹ và buồn nôn..."
            ...
            <div className="mt-4 flex flex-wrap gap-2">
               ...
               {['Đau đầu, chóng mặt', 'Ho kéo dài, tức ngực', 'Đau dạ dày', 'Sốt xuất huyết'].map(tag => (
                 <button 
                  key={tag}
        ...
        <div className="flex flex-col">
          {loading ? (
             <div className="flex-1 bg-white rounded-2xl shadow-sm border border-slate-200 p-8 flex flex-col items-center justify-center text-center animate-pulse">
                ...
                <h3 className="text-lg font-medium text-slate-700">Đang tham khảo ý kiến chuyên gia...</h3>
                <p className="text-slate-400 text-sm mt-2 max-w-xs">Hệ thống đang đối chiếu triệu chứng với cơ sở dữ liệu y khoa.</p>
             </div>
          ) : result ? (
            <div className="flex-1 bg-white rounded-2xl shadow-lg border border-medical-100 overflow-hidden flex flex-col animate-fade-in-up">
              <div className="bg-medical-50 p-6 border-b border-medical-100">
                ...
                <p className="text-slate-500 text-sm mt-1">Mã khoa: <span className="font-mono">{result.deptCode}</span></p>
              </div>
              ...
              <div className="p-6 flex-1 flex flex-col">
                <div className="flex items-start gap-3 mb-6">
                  ...
                  <div>
                    <h4 className="text-sm font-semibold text-slate-700 mb-1">Lý do gợi ý</h4>
                ...
                <div className="flex items-center gap-2 mb-8">
                  ...
                  <span className="text-xs font-medium text-slate-500">{result.confidence}% Phù hợp</span>
                </div>

                <div className="mt-auto pt-4 border-t border-slate-100">
                  <div className="flex items-start gap-2 mb-4 p-3 bg-yellow-50 rounded-lg border border-yellow-100">
                    <AlertCircle className="w-4 h-4 text-yellow-600 mt-0.5 shrink-0" />
                    <p className="text-xs text-yellow-700">Kết quả này chỉ mang tính chất gợi ý điều phối. Bác sĩ chuyên khoa sẽ khám chi tiết.</p>
                  </div>
            ...
            <div className="flex-1 bg-slate-50 rounded-2xl border border-dashed border-slate-300 p-8 flex flex-col items-center justify-center text-center">
               ...
               <p className="text-slate-400">Nhập triệu chứng bên trái và nhấn "Phân tích" để nhận gợi ý khoa khám bệnh.</p>
            </div>
          )}
        </div>
      </div>
